...
期权收益率计算必须分离**时间价值**与**内在价值**，只有时间价值才是卖方的真正收益来源。
...
# 示例：
# 股价 = $130, 行权价 = $100
# 内在价值 = max(0, 100 - 130) = $0 (OTM)
#
# 股价 = $130, 行权价 = $140, 期权价 = $15
# 内在价值 = max(0, 140 - 130) = $10 (ITM)
# 时间价值 = 15 - 10 = $5
```
...
# 示例：
# 股价 = $130, 行权价 = $140
# 内在价值 = max(0, 130 - 140) = $0 (OTM)
#
# 股价 = $130, 行权价 = $120, 期权价 = $15
# 内在价值 = max(0, 130 - 120) = $10 (ITM)
# 时间价值 = 15 - 10 = $5
```
...
# 示例：
# 股价 = $130, 行权价 = $120, 时间价值 = $3, 到期天数 = 30
# 单次收益率 = 3 / 120 * 100 = 2.5%
# 年化收益率 = 2.5% / 30 * 365 = 30.42%
```

#### Sell Call 策略 (Covered Call)

```python
# 单次收益率 = 时间价值 / 当前股价 (持有股票的成本)
...
1. **内在价值不是收益**：ITM 期权的内在价值只是反映了当前股价与行权价的差距，卖方卖出这部分价值后，如果被指派，需要以更不利的价格买入/卖出股票，与收到的内在价值相抵消。

2. **避免误导性高收益**：如果用总权利金计算收益率，ITM 期权会显示出不合理的高年化收益（如500%+），这会误导投资者。

3. **真实反映卖方收益**：时间价值才是期权卖方通过时间衰减（Theta decay）获得的真正收益。

### 筛选规则

```python
# Sell Put: 只筛选 OTM 或轻度 ITM 期权
if strike > current_price * 1.02:  # ITM 超过 2%
    return None  # 跳过深度 ITM 期权

# Sell Call: 只筛选 OTM 或轻度 ITM 期权
if strike < current_price * 0.98:  # ITM 超过 2%
    return None  # 跳过深度 ITM 期权
