...
AlphaGBM 是一个现代化的量化投资分析平台，集成了：

- **股票分析**：基于 AlphaGBM (G=B+M) 模型的智能股票分析
- **期权研究**：实时期权链数据分析和量化评分
...
- ✅ **全新首页设计**：以期权为核心的新首页
  - 核心公式展示：收益 = 基本面 + 情绪
  - 今日热门期权推荐组件
  - 模拟投资组合展示（$100K 老虎证券模拟账户）
  - 快速入口：期权研究、反向查分、股票分析
- ✅ **反向查分功能**：输入期权参数获取算法评分
  - 支持截图上传 + AI识别（Google Gemini Vision）
  - 手动录入期权参数
  - 四种策略评分：Sell Put、Sell Call、Buy Call、Buy Put
  - 风险收益风格标签
- ✅ **热门推荐 API**：
  - `/api/options/recommendations` 端点
  - 每日热门期权推荐
  - 市场总结（趋势、VIX、推荐策略）
- ✅ **模拟投资组合组件**：
  - 展示账户总值、收益、持仓
  - 支持老虎证券 API 数据接入（预留接口）
  - 实时盈亏显示
- ✅ **定价页面更新**：
  - 免费版：每日2次，期权分析，股票分析
  - Plus会员：1000次/月，期权分析，反向查分，股票分析
  - Pro会员：5000次/月，全部功能 + 投资回顾

### v2.2 - 2026年1月20日

#### 期权收入视图重新设计
- ✅ **推荐组合收入预览**：全新的收入视图，展示推荐期权组合的整体收益
  - 三列核心数据：预期总收入、总保证金需求、组合收益率
  - 年化收益率显示：自动计算并显示年化收益率
  - 推荐明细列表：每个期权显示收入、保证金和收益率
  - 风险提示：清晰的 Sell Put/Call 风险说明
- ✅ **专业图标设计**：使用 Bootstrap Icons 替换 emoji，更加专业
  - 稳健风格：shield-check 盾牌图标
  - 平衡风格：sliders 滑块图标
  - 激进风格：lightning-charge 闪电图标
- ✅ **用户体验优化**：
  - 从"一堆期权卡片"到"组合方案"的展示转变
  - 清晰的投入产出比，用户一眼看懂需要投入多少、能赚多少
  - 风险透明：每一笔收入都配对保证金需求

### v2.1 - 2026年1月20日

#### 期权算法优化
- ✅ **年化收益计算修正**：修复了期权年化收益率计算不合理的问题
  - 分离时间价值与内在价值，仅基于时间价值计算收益率
  - PUT期权：内在价值 = max(0, 行权价 - 股价)
  - CALL期权：内在价值 = max(0, 股价 - 行权价)
  - 年化收益 = (时间价值 / 成本基础) × (365 / 到期天数)
- ✅ **权利金显示优化**：期权权利金显示为每手价格（×100），反映美股期权每手100股的实际成本
- ✅ **移除人为上限**：取消了200%年化收益的人为限制，保持计算真实性
